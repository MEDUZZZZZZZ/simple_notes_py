...
def ui_input(key: int):
    match key:
        case 0:
            message = "Добро пожаловать в приложение заметки!\n\
...
\nВведите команду: "
        ...
        case 6:
            message = "Введите:\n1 - 'да'\n2 - 'нет'\n:"
        case 7:
            message = "Введите год дату в формате ггггммдд: "
        case 8:
            message = "Провести поиск с фильтрацией по дате?\n1 - 'да'\n2 - 'нет'\n:"
    return str(input(message))


def convert_to_output(sourse: str):
    tmp = sourse.split(";")
    res = f'id {tmp[0]} Название: {tmp[1]} дата последнего изменения {tmp[3]}Содержание:\n{tmp[2]}\n'
    res += f"{'-'*len(res)}\n"
    return res
    ...
    match flag:
        ...
        case '2':
            for c_note in notes_list:
                res += convert_to_output(c_note)
        case _:
            res = "Ввидимо вы ввели, что-то не так"
    print(res)
...
def ui_show_some(sourse: list[str]):
    count = 0
    res = f'Нашлось {len(sourse)} заметок по вашему запросу\n'
...
def ui_choose_note(sourse: list[str]):
    ...
    if choose <= len(sourse):
        return sourse[choose]
    else:
